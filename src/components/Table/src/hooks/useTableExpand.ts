...
export function useTableExpand(
  ...
  function colseExpandRows(keys: string) {
    const { isTreeTable } = unref(propsRef);
    if (!isTreeTable) return;
    const index = expandedRowKeys.value.indexOf(keys);
    if (index != -1) {
  ...
  return { getExpandOption, expandAll, expandRows, colseExpandRows, collapseAll };
}
