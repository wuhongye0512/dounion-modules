 ...
 * @LastEditTime: 2022-08-31 09:23:07
...
<script lang="ts">
  ...
  import { getUcenterCodeViewApi } from '/@/api/testDemo/sysCode';
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const { close } = useTabs();
      const getView = async () => {
        let res = await getUcenterCodeViewApi({ value: route.params.id });
        console.log(res, 'res');
        res.isSysName = res.isSys ? '系统' : '用户';
        info.value = res;
      };
      getView();
