...
export function useTabs(_router?: Router) {
  ...
  // 获取指定的tab
  function getAppointTab(name: string) {
    return tabStore.getTabList.find((item) => item.name === name)!;
  }

  // 改变指定的tab title
  async function updateAppointTabTitle(title: string, name: string) {
    const canIUse = canIUseTabs;
    if (!canIUse) {
      return;
    }
    const targetTab = getAppointTab(name);
  ...
  /**侵入式-修改 添加刷新指定路由 */
  async function handleRefreshOtherPage(pathName?: string) {
    const canIUse = canIUseTabs;
    if (!canIUse) {
      return;
    }
    await tabStore.refreshOtherPage(router, pathName);
  }
  ...
  return {
    /**侵入式-修改 添加刷新指定路由 */
    refreshOtherPage: (pathName?: string) => handleRefreshOtherPage(pathName),
    ...
    /**侵入式-修改 改变指定的tab title */
    setAppointTitle: (title: string, name: string) => updateAppointTabTitle(title, name),
